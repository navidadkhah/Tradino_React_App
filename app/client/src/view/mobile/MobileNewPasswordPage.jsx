//this component asks for the new password after forgeting the previous password and redirect to login page
...
export default function NewPasswordPage() {
  ...
  if (isloading) {
    button = <MobileButton className={'green-btn'} disabler={isloading} name={'loading'} />
  } else {
    button = <MobileButton className={'green-btn'} disabler={isloading} name={'save'} />
  }
  //<Link to={applicant.mobileSignin} className="mb-nwpass-distance"> {button}</Link>
  //let jknk = <button onClick={handleLogin} className="login-btn" disabled={isloading} >login</button>;
  return (
    <div className="tradino">
      <h1 className="tradino-title">Tradino</h1>
      <div className="mb-newPassPge-content">
        <h className="mb-newPassPge-title">Create New Password</h>
        <p className="mb-newPass-instruction">please enter new password</p>
