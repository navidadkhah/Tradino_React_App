...
Like any other webpage your app starts with the [`app/client/public/index.html`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/client/public/index.html) file. React will render your application into `div#app` .
...
Webpack requires an entry point to your application. Think of it as a door to your source code. In this boilerplate [`app/client/index.js`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/client/index.js) is that entry point. Webpack will access the entire app from this file, transpile the application into ES5 and create small chunks of transpiled code. Only the required chunks will be loaded in the browser so that you don't have to worry about the size of your application.

`app/client/index.js` is one of the biggest files of the boilerplate. It contains all the global setup to make sure your app runs smoothly. Let's break its contents down:
